...
def generate_unpartitioned_dataframe(num_rows: int, alpha:float) -> pd.DataFrame:
    source_values = power_law_numbers(n=num_rows, min_value=1, max_value=num_rows, alpha=alpha)
    target_values = power_law_numbers(n=num_rows, min_value=1, max_value=num_rows, alpha=alpha)
    return pd.DataFrame({"source":source_values, "target":target_values})


def generate_partitioned_dataframe(num_rows: int, alpha:float, partition_count:int) -> pd.DataFrame:
    ...
    partition_sizes = [randint(min_partition_size, max_partition_size) for i in range(0, partition_count -1)]
    partition_sizes.append(num_rows - sum(partition_sizes))
    partition_dfs = []
    for i, partition_size in enumerate(partition_sizes):
        partition_df = generate_unpartitioned_dataframe(num_rows=partition_size, alpha=alpha)
...
if __name__ == "__main__":
    ...
    predefined_components_df = generate_partitioned_dataframe(num_rows=NUM_ROWS, alpha=ALPHA, partition_count=PARTITION_COUNT)
    predefined_components_df.to_csv(root + 'predefined_components_data.csv', index=False)
