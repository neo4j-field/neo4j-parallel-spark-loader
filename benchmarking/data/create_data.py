...
def _generate_spark_dataframe(
    ...
    if "partition_col" in columns: # predefined comps
        raw_data = [_create_row_type_2(source_values=source_values, target_values=target_values, partition_values=partition_values) for _ in range(num_rows)]
    else:
        raw_data = [_create_row_type_1(source_values=source_values, target_values=target_values) for _ in range(num_rows)]
...
def _create_row_type_2(
    source_values: List[str], target_values: List[str], partition_values: Union[str, int]
) -> Dict[str, Union[str, int]]:
...
def generate_bipartite_spark_dataframe(num_rows: int) -> DataFrame:
    source_values = ["a", "b", "c"]
    target_values = ["d", "e", "f"]
    return _generate_spark_dataframe(source_values=source_values, target_values=target_values, columns=BIPARTITE_COLUMNS, num_rows=num_rows)

def generate_monopartite_spark_dataframe(num_rows: int) -> DataFrame:
    source_values = ["a", "b", "c"]
    target_values = ["b", "c", "d"]
    return _generate_spark_dataframe(source_values=source_values, target_values=target_values, columns=MONOPARTITE_COLUMNS, num_rows=num_rows)

def generate_predefined_components_spark_dataframe(num_rows: int) -> DataFrame:
    source_values = []
    target_values = []
    partition_values = []
    return _generate_spark_dataframe(source_values=source_values, target_values=target_values, partition_values=partition_values, columns=PREDEFINED_COMPONENTS_COLUMNS, num_rows=num_rows)
...
if __name__ == "__main__":
    ...
    write_spark_dataframe_to_csv(generate_bipartite_spark_dataframe(num_rows=NUM_ROWS), file_path=root+"bipartite/")
    write_spark_dataframe_to_csv(generate_monopartite_spark_dataframe(num_rows=NUM_ROWS), file_path=root+"monopartite/")
